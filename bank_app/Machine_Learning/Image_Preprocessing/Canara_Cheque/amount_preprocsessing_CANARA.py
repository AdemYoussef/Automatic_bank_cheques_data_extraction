...
def amount_cropping_CANARA(cropped_image):
    amount_image2 = cropped_image[280:520, 1450:2340]
    ...
    lines_amount_image2 = cv2.HoughLinesP(canned_amount_image2, 1, np.pi / 180, 100, minLineLength=300,
                                          maxLineGap=600)  # it was 800
    for line in lines_amount_image2:
        x1, y1, x2, y2 = line[0]
        cv2.line(amount_image2, (x1, y1), (x2, y2), (255, 0, 0), 3)
    # Preprocessing for the extraction of the handwritten numbers one by one
    amount_image2_copy = amount_image2.copy()
    # Taking a matrix of size 5 as the kernel
    kernel = np.ones((3, 4), np.uint8)

    amount_image2_copy_erosion = cv2.erode(amount_image2_copy, kernel, iterations=1)
    ...
    for c in contours_amount:
        ...
        if 20 < h < 70 and 20 < w < 70:
            cv2.rectangle(amount_image2, (x, y), (x + w, y + h), (0, 0, 0), 2)
            ROI_amount = amount_image2[y:y + h, x:x + w]
            cv2.imwrite(
                r'C:\Users\ADEM\Desktop\ESPRIT_Education\4er\PI DS\image preprocessing\CANARA cheque\CANARA amount images\ROI_{}.png'.format(
