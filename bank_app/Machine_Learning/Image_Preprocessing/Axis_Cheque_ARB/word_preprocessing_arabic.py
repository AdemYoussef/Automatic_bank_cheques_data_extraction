...
def word_cropping_arabic(cropped_image):
    words_image = cropped_image[500:1750, 50:5700]
    ...
    lines_img_words = cv2.HoughLinesP(canned_words_image, 1, np.pi / 180, 270, minLineLength=220, maxLineGap=960)
    # Draw lines on the image
    for line in lines_img_words:
        x1, y1, x2, y2 = line[0]
        cv2.line(words_image, (x1, y1), (x2, y2), (255, 0, 0),
                 16)  # 9 is THE BEST but we use 8 so i can keep max words details
    # Erosion to increase words area for detection
    words_image_copy = words_image.copy()
    # Taking a matrix of size 8 as the kernel
    kernel = np.ones((10, 21), np.uint8)
    # Erosion
    words_image_copy_erosion = cv2.erode(words_image_copy, kernel, iterations=5)
